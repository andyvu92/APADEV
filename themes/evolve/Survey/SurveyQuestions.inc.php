...
if(isset($_POST["action"])) { 
	...
	if(isset($_POST["qNum"])) {
		...
		for($a = 1; $a <= $qNum; $a++) {
			...
			array_push($QuestionArray, $optionArr);
			array_push($QuestionArray, $_POST["qType".$a]);
			array_push($QuestionArray, $_POST["qMandatory".$a]);
		}
	}
...
<form method="POST" action="surveyquestions" id="surveyForm">
   <div class="row">
        <div class="col-lg-6">
            <label for="">Please select one survey group:</label>
           <input type="hidden" name="test1" value="create">
